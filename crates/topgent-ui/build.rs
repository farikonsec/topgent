...
/// Decode the application icon once, here, and hand the interface raw pixels.
///
/// The window icon has to be RGBA at run time. Decoding the PNG in the binary
/// would put an image decoder in the dependency set of a security tool for the
/// sake of one 512-pixel square that never changes, so it is decoded at build
/// time instead and included as bytes.
///
/// On Windows the same square is also linked in as a resource. That is what
/// Explorer, the taskbar and the notification centre read; a window icon set at
/// run time reaches none of them, which is why the executable showed the system
/// default.
fn icon() {
    let assets = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("assets");
    let png = assets.join("icon.png");
    println!("cargo:rerun-if-changed={}", png.display());

    let file = std::fs::File::open(&png).expect("the application icon is in the repository");
    let mut reader = png::Decoder::new(BufReader::new(file))
        .read_info()
        .expect("the application icon is a readable PNG");
    let size = reader
        .output_buffer_size()
        .expect("the icon is small enough to buffer");
    let mut pixels = vec![0u8; size];
    let frame = reader.next_frame(&mut pixels).expect("the icon decodes");
    assert!(
        frame.bit_depth == png::BitDepth::Eight,
        "the icon must be 8 bits per channel"
    );
    pixels.truncate(frame.buffer_size());
    // The toolkit wants RGBA. An icon drawn without transparency decodes as
    // RGB, so widen it here rather than keeping a second copy of the same
    // square in the repository just to carry an alpha channel of 255s.
    pixels = match frame.color_type {
        ...
        other => panic!("the icon must be RGB or RGBA, not {other:?}"),
    };

    let out =
        std::path::Path::new(&std::env::var("OUT_DIR").expect("cargo sets OUT_DIR")).to_owned();
    std::fs::write(out.join("icon.rgba"), &pixels).expect("write the icon pixels");
    std::fs::write(
        out.join("icon-size.txt"),
        format!("{},{}", frame.width, frame.height),
    )
    .expect("write the icon dimensions");
    ...
    #[cfg(windows)]
    {
        ...
        resource.set_icon(ico.to_str().expect("the icon path is UTF-8"));
