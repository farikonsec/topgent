...
pub fn sidebar<'a>(current: Panel, report: Option<&Report>, s: Style) -> Element<'a, Message> {
    ...
    for panel in Panel::ALL {
        ...
        let label = row![
            iced::widget::container(text(" "))
                ...
                .height(Length::Fixed(s.type_size(size::BODY) + s.pad(space::SNUG)))
            ...
            crate::glyph::view(panel.icon(), s.type_size(size::EMPHASIS), tint),
            text(panel.label())
                .size(s.type_size(size::BODY))
                .font(if selected {
                    theme::STRONG
        ...
        let control: Element<'a, Message> = iced::widget::tooltip(
            iced::widget::button(label)
                .on_press(Message::Show(panel))
                .style(move |_, status| iced::widget::button::Style {
                    background: Some(
                        if selected || matches!(status, iced::widget::button::Status::Hovered) {
                            p.raised
                        } else {
                            iced::Color::TRANSPARENT
                        }
                ...
                .width(Length::Fill)
                .padding([s.pad(space::TIGHT), s.pad(space::SNUG)]),
            iced::widget::container(text(panel.description()).size(s.type_size(size::MICRO)))
    ...
    // The space under the list used to be empty down to the frame. It carries
    // what the window is doing and how much of the ruleset this host can
    // actually answer, which is the number a reader most needs and had to open
    // a panel to see.
    list = list
        .push(iced::widget::space().height(Length::Fill))
        .push(coverage(report, s));
...
fn coverage<'a>(report: Option<&Report>, s: Style) -> Element<'a, Message> {
    let p = s.palette;
    ...
    Column::new()
        .spacing(s.pad(space::HAIR))
        .padding(s.pad(space::SNUG))
        // One fixed label. The first version swapped between two words on a
        // timer, which reads as flicker rather than as information.
        .push(
            text("COVERAGE")
                .size(s.type_size(size::MICRO))
                .color(p.faint),
        ...
        .push(
            text(format!("{covered}/{total} rules"))
                .font(theme::STRONG)
                .size(s.type_size(size::MICRO))
                .color(tint),
        )
        .push(
            text("Rules without a sensor never fire")
                .size(s.type_size(size::MICRO))
                .color(p.faint),
        )
        .into()
