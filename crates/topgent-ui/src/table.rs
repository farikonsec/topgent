...
fn line<'a>(
    ...
    let usable = (width - gaps - s.pad(space::BASE) * 2.0).max(0.0);
    ...
    for (index, column) in columns.iter().enumerate() {
        ...
        content = content.push(cell_view(&cell, label, portion, s));
        if index + 1 < columns.len() {
...
fn cell_view<'a>(
    ...
    let hint = if cell.is_path {
        "click to copy · right-click to show it in the file browser"
    } else {
        "click to copy"
    };
    tooltip(
