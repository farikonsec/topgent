...
pub struct App {
    ...
    stopping: Option<u32>,
    /// What the last stop attempt said, shown until the next sweep.
...
impl App {
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::Export(redacted) => {
                self.stopped = Some("writing this session to a file...".to_owned());
    ...
    fn view(&self) -> Element<'_, Message> {
            ...
            (Some(report), None) => {
                ...
                let grid = pane_grid(&self.split, move |_, pane, _| {
                    pane_grid::Content::new(match pane {
                        ...
                        Pane::Detail => {
                            container(panels::view(panel, report, selected, tables, draft, s))
                ...
                row![panels::sidebar(panel, Some(report), s), grid]
        ...
        if let Some(outcome) = &self.stopped {
            window = window.push(agents::footnote(outcome, s));
        }
    ...
    fn hand_off(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::Copy(value) => {
                self.stopped = Some(format!("copied: {}", table::shorten(&value, 90, true)));
    ...
    fn policy(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::AddRule if self.draft.path.trim().is_empty() => {
                self.stopped = Some("a rule needs a path to watch".to_owned());
                Task::none()
            }
            Message::AddRule => {
